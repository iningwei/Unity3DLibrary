...
public class OpenByNotepadPP : MonoBehaviour
{
    ...
    [MenuItem("Assets/使用Notepad++打开", false, 1)]
    static void  openByNotepadPP()
    {
        ...
        if (!(path.EndsWith(".cs") || path.EndsWith(".txt") || path.EndsWith(".bin") || path.EndsWith(".xml")))
        {
            UnityEngine.Debug.LogError("you can only open .cs .txt .bin .xml file");
            return;
        }         
